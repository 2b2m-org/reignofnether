...
public class DrownedUnit extends Drowned implements Unit, AttackerUnit {
    ...
    public static final EntityDataAccessor<Integer> scenarioRoleDataAccessor =
            SynchedEntityData.defineId(DrownedUnit.class, EntityDataSerializers.INT);
    ...
    public boolean doHurtTarget(@NotNull Entity pEntity) {
        if (super.doHurtTarget(pEntity)) {
            if (pEntity instanceof LivingEntity)
                ((LivingEntity)pEntity).addEffect(new MobEffectInstance(MobEffectRegistrar.ZOMBIE_INFECTED, CONVERT_DEBUFF_DURATION_SECONDS * 20, 0), this);
            return true;
        } else {
            return false;
        }
