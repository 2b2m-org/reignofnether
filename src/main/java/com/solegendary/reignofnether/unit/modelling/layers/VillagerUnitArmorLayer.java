...
public class VillagerUnitArmorLayer<T extends LivingEntity, M extends HumanoidModel<T>, A extends HumanoidModel<T>> extends RenderLayer<T, M> {
            ...
            if (armoritem.getEquipmentSlot() == slot) {
                ...
                for (int layerIndex = 0; layerIndex < material.layers().size(); layerIndex++) {
                    ...
                    int color = extensions.getArmorLayerTintColor(
                            itemstack, entity, layer, layerIndex, fallbackColor
                    );
                    if (color != 0) {
                        this.renderModel(
                                poseStack,
                                ...
                                armorModel,
                                color,
                                ClientHooks.getArmorTexture(entity, itemstack, layer, innerTexture, slot)
                        );
                    }
                }
    ...
    private void renderModel(PoseStack p_289664_, MultiBufferSource p_289689_, int p_289681_, Model p_289658_, int color, net.minecraft.resources.ResourceLocation armorResource) {
